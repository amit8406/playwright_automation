...
const { loginAndGoToEvents } = require('../utils/LoginHelper');
...
test('Mock Events API with 4 events', async ({ page }) => {
    ...
    const banner2 = page.getByText(/sandbox holds up to/i);
    await expect(banner2).not.toBeVisible();
    await expect(page.getByText(/sandbox holds up to/i)).toHaveCount(0);
})
